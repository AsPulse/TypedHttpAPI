...
test('parsePostRequest', async () => {
  expect(await parseType(parse.find(v => v.name === 'POSTRequest')?.value ?? '', p)).toBe(
    'rt.Record({a:rt.Array(rt.String),d:rt.Number,hello:rt.Array(rt.Literal(\'world\'))})',
  );
});

test('parsePostResponse', async () => {
  expect(await parseType(parse.find(v => v.name === 'POSTResponse')?.value ?? '', p)).toBe(
    'rt.Record({b:rt.String,style:rt.Union(rt.Literal(\'js\'),rt.Literal(\'ts\'))})',
  );
});

test('parseUnionOuter', async () => {
  expect(await parseType(parse.find(v => v.name === 'UnionOuter')?.value ?? '', p)).toBe(
...
test('parseCross', async () => {
  expect(await parseType(parse.find(v => v.name === 'Cross')?.value ?? '', p)).toBe(
    'rt.Intersect(rt.Union(rt.String,rt.Number),rt.Boolean)',
  );
});

test('parseCross2', async () => {
  expect(await parseType(parse.find(v => v.name === 'Cross2')?.value ?? '', p)).toBe(
    'rt.Union(rt.Intersect(rt.String,rt.Number),rt.Boolean)',
  );
});

test('parseCross3', async () => {
  expect(await parseType(parse.find(v => v.name === 'Cross3')?.value ?? '', p)).toBe(
    'rt.Intersect(rt.Intersect(rt.String,rt.Union(rt.Number,rt.Boolean)),rt.Null)',
  );
});

test('parseMarto', async () => {
  expect(await parseType(parse.find(v => v.name === 'Marto')?.value ?? '', p)).toBe(
    'rt.Record({a:rt.Record({b:rt.String})})',
  );
});

test('parseRefTest', async () => {
  expect(await parseType(parse.find(v => v.name === 'RefTest')?.value ?? '', p)).toBe(
    'rt.Record({a:rt.Boolean})',
  );
});

test('parseRecordTest', async () => {
  expect(await parseType(parse.find(v => v.name === 'RecordTest')?.value ?? '', p)).toBe(
    'rt.Record(rt.String,rt.Never)',
  );
});

test('parseResolveTest1', async () => {
  expect(await parseType(parse.find(v => v.name === 'ResolveTest1')?.value ?? '', p)).toBe(
    'rt.String',
  );
});
